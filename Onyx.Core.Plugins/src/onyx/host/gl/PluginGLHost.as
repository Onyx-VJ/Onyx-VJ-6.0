...
//			var err:PluginGL			= data as PluginGL;
//			
//			plugin.dataFrag				= err.defaultFragment ? err.defaultFragment.concat() : null;
//			plugin.dataVert				= err.defaultVertex ? err.defaultVertex.concat() : null;
//			
//			const params:Vector.<PluginGLParameter>	= new Vector.<PluginGLParameter>(err.parameters.length, true);
//			
//			var count:int = 0;
//			for each (var def:PluginGLParameterDef in err.parameters) {
//				params[count++] = new PluginGLParameter(def, def.program === Context3DProgramType.FRAGMENT ? plugin.dataFrag : plugin.dataVert);
//			}
			
			// create the parameters
			return Vector.<IParameter>([]);
		}	
